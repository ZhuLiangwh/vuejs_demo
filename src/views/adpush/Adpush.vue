<template>  
	...
	<a class="waves-effect waves-light btn">添加广告</a>
		<table class="highlight">
			...
			<tbody>
				<tr v-for="item in items">
					...
					<td>{{item.id}}</td>
					<td>{{item.title}}</td>
					<td>{{item.rule_id.title}}</td>
					<td>{{{item.usertaglist}}}</td>
					<td>{{item.adpos.title}}</td>
					<td>{{{item.adgroup}}}</td>
					<td>{{item.last_release_ec2}}</td>
					<td>{{item.last_offline_ec2}}</td>
					<td>{{item.is_upload_local}}</td>
					<td>{{item.is_upload_ec2}}</td>
		...
		<ul class="pagination" v-if="totalNum > 0">
...
<script>
import Vue from 'vue'
import adpushlist from '../adpush/adpush.js'
...
export default {
	...
	ready: function() {
		...
		adpushlist.listData(params).then(function(data){
